#!/usr/bin/env python
# Original work, all credit:
# https://qiita.com/rukihena/items/82266ed3a43e4b652adb
#
# Added shitty implementation of graphite writer
# and translated the comments in the code
...
# Define your carbon server
CARBON_SERVER = '192.168.0.250'
CARBON_PORT = 2003

# B-route authentication ID (comes by actual mail from TEPCO)
rbid = "xxx"
# B-route authentication ID (comes by email)
rbpwd = "xxx"
# serial port device name (comment in the one you need)
#serialPortDev = 'COM3'  # Windows
serialPortDev = '/dev/ttyUSB0'  # Linux (like raspi)
#serialPortDev = '/dev/cu.usbserial-A103BTPR'    # Mac
...
while not scanRes.has_key("Channel") :
    ...
    # scan once, loop until the end of the scan
    scanEnd = False
    while not scanEnd :
        line = ser.readline()
        print(line, end="")
...
while True:
    ...
    if line.startswith("ERXUDP") :
		...
		# Format message for carbon

		timestamp = int(time.time())
		message = 'tepco.totalw ' + '%s %d\n' % (intPower, timestamp)
		print(message)

		# Open socket to carbon server, write intPower
